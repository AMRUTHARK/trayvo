  ...
  const fetchReturns = async () => {
    try {
      ...
      if (error.response?.status >= 500 || error.request) {
        toast.error('Failed to fetch purchase returns. Please try again.');
      }
      setReturns([]);
    } finally {
      setLoading(false);
              ...
              <tbody className="divide-y divide-gray-200">
                {returns.map((returnItem) => (
                  <tr key={returnItem.id}>
                    <td className="px-4 py-3 text-sm">{returnItem.purchase_return_number}</td>
                    <td className="px-4 py-3 text-sm">{returnItem.purchase_number}</td>
                    <td className="px-4 py-3 text-sm">{returnItem.supplier_name || '-'}</td>
