...
export default function PurchaseReturnDetailPage() {
  ...
  const fetchReturn = async () => {
    try {
      const response = await api.get(`/purchase-returns/${params.id}`);
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold text-gray-900">Purchase Return Details</h1>
          <button
        ...
        <div className="bg-white rounded-lg shadow p-8">
          <div className="border-b border-gray-200 pb-6 mb-6">
            <div className="flex justify-between items-start">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{returnData.purchase_return_number}</h2>
                <p className="text-gray-600 mt-1">
                  {format(new Date(returnData.created_at), 'dd MMM yyyy, HH:mm')}
                </p>
                <p className="text-sm text-gray-500 mt-1">Purchase: {returnData.purchase_number}</p>
              </div>
            </div>
          </div>
          ...
          {returnData.supplier_name && (
            <div className="mb-6">
              ...
              <div className="text-gray-600">{returnData.supplier_name}</div>
            </div>
          )}
